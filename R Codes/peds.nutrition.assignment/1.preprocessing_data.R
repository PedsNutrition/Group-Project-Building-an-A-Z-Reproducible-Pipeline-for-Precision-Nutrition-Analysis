...
setwd("C:/Users/ameth/Documents/GitHub/Group-Project-Building-an-A-Z-Reproducible-
      Pipeline-for-Precision-Nutrition-Analysis/R Codes/peds.nutrition.assignment")

#Import dataset to Global Environment: 
library(readr)
mock_precision_growth_dataset <- read_csv("mock_precision_growth_dataset.csv")
View(mock_precision_growth_dataset)
data <- mock_precision_growth_dataset #making a new dataframe for preprocessing
...
#replace zbmi_24 to zBMI_24m to match the exact column name in dataframe
...
#replace crp to CRP to match the exact column name in dataframe
...
# Compare with how many values are at or below 10 (not the extreme values)
sum(data$CRP <= 10, na.rm = TRUE)
...
library(mice) #load the packages

#Step 1: Detect variable classes and levels
is_numeric <- sapply(clean_data, is.numeric)
is_factor  <- sapply(clean_data, is.factor)

#Step 2: Build mice method vector and predictor matrix
meth <- make.method(clean_data)
pred <- make.predictorMatrix(clean_data)
subject_var <- "ID"
target_var   <- "zBMI_24m"  

#Step 3: Set variables that will not be imputed (IDs or complete variables)
vars_no_impute <- c( 
  "ID",
  "Sex",
...
pred[, "ID"] <- 0              # ID does not help predict
pred[, "zBMI_24m"] <- 1          # allow outcome to help impute predictors
pred["zBMI_24m", ] <- 0          # but the outcome itself is not imputed

#Step 2: Assign methods based on type and number of levels
...
#Step 3: Run multiple imputations
library(mice)
...
#number of imputation (m) = 10, because it only 10% missing data.
# maximum iteration (maxit) = 50, to not overkill the natural origin.

# Check how many NAs remain after imputation
...
# Revise code, because there are still NAs
# Initialize defaults
meth <- make.method(clean_data)
pred <- make.predictorMatrix(clean_data)

# Force imputation method for these variables (pmm = predictive mean matching, good for continuous)
meth["ALT"]               <- "pmm"
meth["Fiber_intake_g"]    <- "pmm"
meth["Shannon_diversity"] <- "pmm"
meth["Weight_12m_kg"]     <- "pmm"

# Make sure they are used as targets in the predictor matrix (row != 0)
pred["ALT", ]               <- 1
pred["Fiber_intake_g", ]    <- 1
pred["Shannon_diversity", ] <- 1
pred["Weight_12m_kg", ]     <- 1
diag(pred) <- 0  # a variable cannot predict itself
...
# Step 4: Diagnostics of Imputed Variables
