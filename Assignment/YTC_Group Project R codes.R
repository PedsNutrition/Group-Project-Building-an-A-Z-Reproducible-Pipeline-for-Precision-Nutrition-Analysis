...
# Compute k-means with optimal_k
set.seed(123)
km.res <- kmeans(df, optimal_k, nstart = 25)
print(km.res)

# Compute the mean zBMI of each cluster
aggregate(df_full, by = list(cluster = km.res$cluster), mean)

dd <- cbind(df_full, cluster = km.res$cluster)
head(dd)
...
# Compute PAM with optimal_k
pam.res <- pam(df, optimal_k)
print(pam.res)

dd <- cbind(df_full, cluster = pam.res$cluster)
head(dd, n = 3)

# Visualize PAM results — boxplot instead of fviz_cluster
# since we only have 1 variable (zBMI_24m)
pam_df <- data.frame(
  zBMI_24m = df_full$zBMI_24m,
...
# Running Hopkins test to test whether data has meaningful cluster structure. Hopkins Package
# was installed in 2. dist_computation but was never ran. 
set.seed(123)
hopkins_stat <- get_clust_tendency(df.scaled, n = 50, graph = FALSE)
cat(sprintf("\nHopkins statistic: %.3f\n", hopkins_stat$hopkins_stat))
cat("Interpretation:",
    ifelse(hopkins_stat$hopkins_stat > 0.5,
           "Data shows cluster tendency - worth clustering",
           "Data may be random - clustering may not be meaningful"), "\n")

# ==========================================================================
# CLUSTER PROFILING (OWN R CODE)
# ==========================================================================
# In this section, we attach the k-means cluster labels back to the full 
# dataset to profile maternal factors across the three identified growth trajectory clusters. 
# This allows us to examine whether Ultra_processed_score, Household_income_index, and 
# Energy_intake_kcal differ between children with lower, normal, 
# and higher zBMI at 24 months. This directly addresses the research question:
# "Are maternal dietary and socioeconomic factors associated with early 
# childhood growth trajectories as defined by zBMI-based cluster patterns 
# at 24 months of age?

# Attach k-means cluster labels back to full dataset
data_clustered <- data[!is.na(data$zBMI_24m), ]
data_clustered$cluster <- km.res$cluster

# Label clusters by growth trajectory
data_clustered$cluster_label <- ifelse(data_clustered$cluster == 1, "Normal Growth",
                                       ifelse(data_clustered$cluster == 2, "Lower Growth",
                                              "Higher Growth"))

# Compare maternal predictor variables across clusters
cat("=== Mean Predictor Values by Cluster ===\n")
aggregate(data_clustered[, c("Ultra_processed_score",
                             "Household_income_index",
                             "Energy_intake_kcal")],
          by = list(Cluster       = data_clustered$cluster,
                    Growth_Group  = data_clustered$cluster_label),
          FUN = mean)